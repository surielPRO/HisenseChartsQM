...
const fetchGoogleSheetData = async (sheetName) => {
  try {
    ...
    return []; // devolvemos [] para evitar que la app se rompa
  }
};
      ...
      <div className="content">
        ...
        <div className="second-row" style={{ gap: "10px" }}>
          <div className="glass-card issues-card" style={{ width: '50%', height: '190px' }}>
            <h3>{`${currentMonth} TOP ISSUES ${currentYear} - INJECTION`}</h3>
          ...
          <div className="glass-card issues-card" style={{ width: '50%', height: '190px' }}>
            <h3>{`${currentMonth} TOP ISSUES ${currentYear} - STAMPING`}</h3>
